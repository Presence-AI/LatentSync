...
from .worker import (
    ...
    denoise_worker, launch_workers, 
    cleanup_workers, 
    restore_video_worker,
    initialize_workers
)
...
class LipsyncPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print("num inference step: ", num_inference_steps)
        ...
        # Keep fetching until all batches are processed
        while completed_batches < num_inferences:
            batch_id, restored_batch = self.final_queue.get()
            print(f"Get {batch_id} after {time.time() - start_time}")
            if restored_batch is None:
                raise RuntimeError(f"Error occurred in worker for batch {batch_id}")
