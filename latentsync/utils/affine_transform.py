...
class AlignRestore(object):
    ...
    def restore_img(self, input_img, face, affine_matrix):
        try:
            ...
            input_img = rearrange(
                torch.from_numpy(input_img).to(device=self.device, dtype=self.dtype),
                "h w c -> c h w",
            )
            ...
            inv_mask_erosion = (
                inv_mask_erosion.squeeze().cpu().numpy().astype(np.float32)
            )
            ...
            img_back = (
                inv_soft_mask_3d * pasted_face + (1 - inv_soft_mask_3d) * input_img
            )

            img_back = (
                rearrange(img_back, "c h w -> h w c").contiguous().to(dtype=torch.uint8)
            )
